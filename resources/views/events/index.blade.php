        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($events as $event)
                <div class="bg-white p-4 rounded-lg shadow-lg">
                    ...
                    <a href="{{ route('events.show', $event->id) }}" class="text-blue-500 hover:underline mt-2 block">View Details</a>

                    @if(auth()->user() && auth()->user()->role === 'admin')
                        <div class="mt-2">
                            <a href="{{ route('events.edit', $event->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded-md">Edit</a>
